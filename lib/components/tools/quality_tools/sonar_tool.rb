...
module MGT
  class SonarQubeTool
    ...
    def initialize args
      ...
      @url = URI(args.URL)
      @tool_name = 'Sonarqube'
      @sonar_web_context = args.sonar_web_context || ''
    end

    ##
    # check for connecting with incoming configuration parameters. Also check for server side response
    def test_connection
      path = @project ? "#{@sonar_web_context}/api/resources?resource=#{@project}" : '#{@sonar_web_context}/api/resources'
      try(path)
    end
    ...
    def path_with_project_name
      if @project
        "#{@sonar_web_context}/api/resources?resource=#{@project}&metrics=ncloc,duplicated_lines,blocker_violations,critical_violations,major_violations"
      else
        '#{@sonar_web_context}/api/resources?metrics=ncloc,duplicated_lines,blocker_violations,critical_violations,major_violations'
      end
    end
